...
class BinarySearchTree {
 ...
 // ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if(!node){
    ...
    else if (less(item, node->datum) && node->left == nullptr) {
      node->left = new Node(item, nullptr, nullptr);
    }

    else if (!less(item, node->datum) && node->right == nullptr) {
      ...
      // must be !less bc no duplicates invariant 
      else {
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (less(max_element_impl(node)->datum, val)) {
      //std::cout << max_element_impl(node)->datum << " < " << val  << std::endl;
      //std::cout << "if 2" << std::endl;

      return nullptr;
    }

    // min greater than val is in the right branch (or DNE)
    if (less(node->datum, val)) {

      return min_greater_than_impl((node->right), val, less);
    }

    // in left branch or CURRENT NODE!!! or dne
    ...
    // never get here
    assert(false);
    return nullptr; // shut up compile error there's a return outside my ifs
  }
