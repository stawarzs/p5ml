...
private:
  ...
  public:
    ...
    bool operator() (Pair_type pair1, Pair_type pair2) const {
      return comparison(pair1.first, pair2.first);
    }
  private:
    Key_compare comparison;
  };
...
public:
  ...
  Value_type& operator[](const Key_type& k){
    ...
    //my version with entries. added before end() and insert
    //if element is not present
      if(entries.find({k, Value_type()}) == entries.end()){
...
/*
    //std::pair<k, Value_type> arbitrary;
    
    //if element is not present
      if(entries.find({k, Value_type()}) == end()){
        insert({k, Value_type()});
        return find(k)->second;
    }
    else {
...
*/
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){ 
    ...
    auto searchKey = entries.find(val);

    //if element present
    if(searchKey != entries.end()){
    return std::pair<Iterator, bool>{entries.find(val), false};
    }
    
    //if element not present
    else {
      return std::pair<Iterator, bool> {entries.insert(val), true};
    }
  }
